package day18;

public class Password {
    public static void main(String[] args) {
        System.out.println(genPass(10, "l", "s"));
        System.out.println(genPass(20, "l", "s"));
        System.out.println(genPass(30, "l", "s"));
    }

    private static String genPass(int length, String a, String b) {
        String output = "";
        for (int i = 0; i < length; i++) {
            output= output + getMeLetter(a, b );

        }


        return output;
    }

    private static char getMeLetter(String a, String b) {
        String sourceC = "ывлвьвлвьлвлтвоуоушщуьтвтьвбдвдоутуьлст";
        String sourceDigit = "1837568490873";
        String sourceSymbol = "§$%&/)=?ß";
        String sourceL = "dkeijcnkdlldmmjeiolm";
        String sourceAll = sourceL + sourceC + sourceDigit + sourceSymbol;
        if (a.equals("l")){
            sourceAll = sourceL;
        }
        if (b.equals("s")){
            sourceAll = sourceAll + sourceSymbol;
        }
        int min = 0;
        int max = sourceAll.length() - 1;
        int result = (int) ((Math.random() * (max - min) + 1) + min);
        char output= sourceAll.charAt(result);
        return output;


    }
}
